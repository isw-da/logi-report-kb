...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45203886502797-Synchronizing-the-Data-Components-in-a-Dashboard
updated_at: 2026-04-30T14:08:08Z
...
You can also predefine to send and receive messages when you create library components in Report Designer. For more information, see Delivering Messages Between Library Components in the Report Designer Guide.
...
If you want to view or edit the message a data component automatically receives, you can make use of its Receive Sync dialog box. While for the Parameter message, if a library component contains several data components, JDashboard displays the dialog box only on the first data component in the library component.

### 
Sending Out a Customized Message

- Right-click a trigger object and select Send Sync > Customize from the shortcut menu. JDashboard displays the Send Sync dialog box. 
    

- In the message box on the left, select   to add a message line, select the type of the message from the Message Type drop-down list, which can be one of the built-in messages Filter, Sort, Parameter, and On-screen Filter, or User Defined. JDashboard displays a message name automatically in the Message Name column. You can double-click in the column to edit the message name.
    If the object has been predefined to send messages in Report Designer, JDashboard will list them in the message box, marking with , and you cannot modify them.

- In the message information box on the right, define the message.
...
- Select  above the message information box to add a message key-value line.

- In the Key Name column, specify a key from the drop-down list, or select the <Input...> item from the list and then type a key in the text box. To make the condition dynamic, select the values under the Dynamic Key node.

- In the Value column, specify the value of the key from the drop-down list, or select <Input...> from the list and then type a value into the text box. To make the condition dynamic, select the values under the Dynamic Key node.

- You can add more key-value lines and specify the key and value in each line.
            To delete a message key-value line, select it, and then select .

Tip: You can also create a blank user defined message without specifying the key and value. For a bubble chart, by default, current field and current value mean the bubble radius.

- Repeat the preceding two steps to add more messages on the object.
    To delete a message, select it, and then select  above the message box. You cannot remove messages predefined in Report Designer.
...
- Right-click anywhere on a data component and select Receive Sync from the shortcut menu. Server displays the Receive Sync dialog box.
    

If the data component has been predefined to receive messages in Report Designer, JDashboard  will list them in the dialog box and mark them with .

- Select  to add a message line.

- The Sync From drop-down list displays all the trigger objects in the current dashboard. Select the trigger object that sent out the message which you want the data component to receive.

- JDashboard lists the messages that have been defined on the selected trigger object, in the Message Name drop-down list. Select the message the data component is going to receive. If the selected message is a built-in message, JDashboard displays the action that is bound with the message, in 
 the Actions column.

- You can double-click in the Action Description column to add a description for the action the message will respond.

- Specify how you want the data component to respond when it receives the message.
    
- If the to be received message is a built-in Filter message, the data component will filter itself according to the filter conditions defined in the message. You can further edit the filter conditions if you want.

- If the to be received message is a built-in Sort message, the data component will sort itself according to the sort conditions defined in the message. You can further edit the sort conditions if you want.

- If the to be received message is a built-in Parameter message, the data component will re-run using the parameter values in the message. You can further edit the parameter values if you want.

- If the to be received message is a built-in On-screen Filter message, the data component will filter itself according to the filter conditions defined in the message. You can further edit the filter conditions if you want.

- If the to be received message is a user defined message, you can specify the web action the data component will respond to when it receives the message and define how to perform the action. You can specify the following web actions: Filter, On-screen Filter, Remove Filter, Sort, Parameter, Property, Go Down, Go Up, and Go To.

- Repeat the preceding steps to add more messages to receive and define how to respond to the messages.
    To delete a message, select it, and then select  above the message information box. You cannot remove the messages that were predefined in Report Designer.
...
- Select OK to accept the settings and close the dialog box.
 ...
 In the drop-down lists for editing the conditions to respond a message, you may find that some items under the Message Key node have the icon  ahead of them. If you choose an item of this type, JDashboard will not use the item directly in a condition, instead map it to a key in the message. JDashboard will find the value that is specified to the key from the message first and then use that value to compose the condition.

To edit the filter conditions for the built-in Filter message:

- In the Receive Sync dialog box, select  in the Actions column. JDashboard displays the Web Action - Filter dialog box.
    

- Specify the filter condition by selecting the field on which the filter will be based from the Filter On drop-down list, the operator to compose the condition from the Operator drop-down list, and the value of how to filter the field from the Value drop-down list.
...
- You can select  to add more filter conditions to filter other field values and then specify the relationship between the conditions in the More column.
    To delete a filter condition, select the condition and then select .

- Select OK to finish defining the filter conditions.

To edit the sort conditions for the built-in Sort message:

- In the Receive Sync dialog box, select  in the Actions column. JDashboard displays the Web Action - Sort dialog box.
...
- You can select  to add more sort conditions to sort other fields.
    To delete a sort condition, select the condition and then select .

- Select OK to finish defining the sort conditions.

To edit the parameter values for the built-in Parameter message:

- In the Receive Sync dialog box, select  in the Actions column. JDashboard displays the Web Action - Parameter dialog box.
    

- Specify the parameter and the value of the parameter from the Parameter Name and Parameter Value drop-down lists. The operator can only be =.
...
- Select  to add more parameter lines to specify other parameter values. If you specify a parameter value more than once, the last setting takes effect.
    To delete a parameter line, select it, and then select .

- Select OK to accept the settings. Then at runtime, when the data component receives the message, it will re-run itself, using the predefined parameter values.
            

To edit the filter conditions for the built-in On-screen Filter message:

- In the Receive Sync dialog box, select  in the Actions column. JDashboard displays the Web Action - On-screen Filter dialog box.
    

- Specify the filter condition by selecting the field on which the filter will be based from the Filter On drop-down list and the value of how to filter the field from the Value drop-down list.
    If you choose to type multiple values manually, you should separate them with ",", and if "," or "\" is contained in the values, write it as "\," or "\\".

- You can select  to add more filter conditions to filter other field values.
    To delete a filter condition, select the condition and then select .
...
- In the Receive Sync dialog box, select *Filter from the drop-down list of the Actions column, then select  in the Actions column.

- In the Web Action - Filter dialog box, specify the filter condition.
...
- In the Receive Sync dialog box, select *On-screen Filter from the drop-down list of the Actions column, then select  in the Actions column.

- In the Web Action - On-screen Filter dialog box, specify the filter condition.
...
- In the Receive Sync dialog box, select *Remove Filter from the drop-down list of the Actions column, then select  in the Actions column. JDashboard displays the Web Action - Remove Filter dialog box.
    

- Specify the filters you want to remove from the data component: to remove the filters that are created via the Filter option on a field’s shortcut menu, the Filter web action, and the Filter dialog box, select Remove Filters; to remove the on-screen filters created by the On-screen Filter web action, select Remove *On-screen Filters.

- By default, JDashboard will remove all the on-screen filters from the data component when you select Remove *On-screen Filters. If you want to only remove the filter conditions that are related to certain data fields, clear the All Fields option, then select  to add a condition line, select in the line and select a field from the drop-down list or select <Input...> and type the field name manually. You can select  to add more fields. To remove an unwanted field, select it, and then select .
...
- In the Receive Sync dialog box, select *Sort from the drop-down list of the Actions column, then select  in the Actions column.

- In the Web Action - Sort dialog box, specify the sort condition.
...
- In the Receive Sync dialog box, select *Parameter from the drop-down list of the Actions column, then select  in the Actions column.

- In the Web Action - Parameter dialog box, specify the parameters.
...
- In the Receive Sync dialog box, select *Property from the drop-down list of the Actions column.

- Select  in the Actions column. JDashboard displays the Web Action - Change Property dialog box.
    

- Specify the property you want to change and the value for it respectively from the Properties and Value drop-down lists. You can only change the properties of the data component itself.

- You can select  to add more property lines to change the values of other properties.To delete a property line, select it, and then select .
...
- In the Receive Sync dialog box, select *Go Down from the drop-down list of the Actions column.

- Select  in the Actions column. JDashboard displays the Web Action - Go Down dialog box.
    

- In the Go Down On column, select the group on which the go-down action will occur. For a geographic map, it is Current Layer and you cannot change it.

- In the Use Hierarchy column, select the hierarchy based on which the go-down action will occur. 
...
- In the By Value column, type a value of the specified group field to go down to the lower level with the filter condition SpecifiedGroupField=TheValue. You can also use a web control to retrieve a value.
...
- In the Receive Sync dialog box, select *Go Up from the drop-down list of the Actions column.

- Select  in the Actions column. JDashboard displays the Web Action - Go Up dialog box.
    

- In the Go Up On column, select the group on which the go-up action will occur. For a geographic map, it is Current Layer and you cannot change it.

- In the Use Hierarchy column, select the hierarchy that the go-up action will occur according to. You can also use a web control in the dashboard to retrieve a hierarchy name. For a geographic map, JDashboard applies the Built-in hierarchy and you cannot change it.
...
- In the Receive Sync dialog box, select *Go To from the drop-down list of the Actions column.

- Select  in the Actions column. JDashboard displays the Web Action - Go To dialog box.
    

- In the Go To On column, select the field on which the go-to action will occur.

- In the To Column column, select another field to change the Go To On field to. You can also use a web control in the dashboard to retrieve a field name.

- To perform the go-to action by filtering the Go To On field at the same time, select By Value and then type the value by which to filter the field or select a web control to retrieve the value at runtime. Skip this step if you select to perform the action on a crosstab aggregate field.
