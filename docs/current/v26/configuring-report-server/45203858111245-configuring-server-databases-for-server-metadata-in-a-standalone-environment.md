...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45203858111245-Configuring-Server-Databases-for-Server-Metadata-in-a-Standalone-Environment
...
This topic describes the two ways of configuring server databases for server metadata in a standalone environment: via the Report Server UI or using the configuration file dbconfig.xml.
...
- On the system toolbar of the Server Console, navigate to Administration > Configuration > Server DB > System DB/Realm DB/Profiling DB. Server displays the corresponding page. 

- Select a realm if you are configuring the realm or profiling database from the Select Realm drop-down list at the upper-right corner.
...
You can configure the server database in dbconfig.xml in <install_root>\bin using either of the following two methods. 
 ...
 In dbconfig.xml, <database name> must be Report Server's inner database name: systemtables for the system database, realmtables for the realm database, or profile for the profiling database.
...
You can modify the configuration via this method on the Server Console.
...
- 
Report Server encrypts the <user> and <password> information. It replaces them with the <encrypt-nsign> tag after it starts:                    <encrypt-nsign>enDkq7srM9cHhoUwzYXJ3NvcDIYk</encrypt-nsign>
...
- Because of the compatibility between the third-party package Quartz in Report Server and the driver of Oracle database, the Quartz package throws an exception when you add the class path of the Oracle database driver into dbconfig.xml. To avoid the problem, when Report Server works with an Oracle database, do not set the database driver class path in dbconfig.xml.
