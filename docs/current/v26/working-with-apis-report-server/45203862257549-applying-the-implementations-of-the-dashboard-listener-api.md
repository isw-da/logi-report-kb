...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45203862257549-Applying-the-Implementations-of-the-Dashboard-Listener-API
...
For more information, see the com.jinfonet.web.modules.dashboard package in the Report Javadoc. You can also refer to the sample implementation file DashboardListenerDemo.java in <install_root>\help\samples\APIDashboardListener for additional information.
...
- On the system toolbar of the Server Console, navigate to Administration > Other > Dashboard Listeners. Server displays the Dashboard Listeners page.
    

- Select New Dashboard Listener. Server displays the New Dashboard Listener dialog box.

- Select Browse to locate the implementation file which could be a zip or a jar file. 

- Select Next.

- From the Class Name list, choose a class name which you defined in the implementation file.
    

- 
Select the Add button  beside the Target box. Server displays the Select Target dialog box. 

- Specify the target dashboards you would like the implementation to apply to: all dashboards including the unsaved ones, the newly created dashboards that you have not saved, or dashboards in the specified folders.
    

-  Select OK. Server lists the specified dashboards or folders in the Target box.

    When you have added all dashboards, Server disables the  button. Later if you want to change the dashboards, you need to first delete the <All dashboards> item, and then add other dashboards. To remove unwanted dashboards from the Target box, select them and then select the Remove button  . 

- In the Description text box, you can type a description for the implementation.

- Select Enabled to enable the implementation.

- Select Finish. The registered class will take effect right away. 
...
-  To modify the target value of an implementation, select it, then select Properties on the toolbar or right-click and then select Properties from the shortcut menu. Server displays a dialog box, and you can modify the target dashboards. 
...
To apply the dashboard listener via URL, append the key-value pair jrd_dsh_listener=PackageName.ClassName in the URL for running dashboards. 
