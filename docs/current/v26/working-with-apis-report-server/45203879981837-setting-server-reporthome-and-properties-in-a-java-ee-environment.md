...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45203879981837-Setting-Server-Reporthome-and-Properties-in-a-Java-EE-Environment
...
You can use the interface jet.server.api.http.CustomizedServerEnv to specify the Report Server reporthome and to set the server properties in a Java EE environment. This topic describes the interface CustomizedServerEnv and how you can apply the default and customized implementations of the interface.
...
Report Server will retrieve the reporthome and properties from your implemented class when the WAR/EAR file is deployed.
...
The self-contained Report Server provides two implementations of CustomizedServerEnv. They are jet.server.DefaultServerEnv and jet.server.MultipleInstanceServerEnv.
...
This is an internally implemented class of the jet.server.api.http.CustomizedServerEnv interface which supports multiple Report Server instances in one Java EE application server. The class can automatically assign the reporthome of each instance.
...
- The Report Server must be initialized with JRServerContextListener from a Web module.
    In cases of deploying multiple Report Server instances in one Java EE application server without touching the WAR, such as extracting the WAR, setting reporthome, and rebuilding the WAR, Report Server has to use ServletContext to generate an ID for every instance. Report Server retrieves javax.servlet.context.tempdir from ServletContext by invoking the method getAttribute(String), and then uses this value to generate the instance ID.

For more information, see Java Servlet Specification Version 2.3/2.4 SRV.3.7.1 Temporary Working Directories.

- 
Report  Server must create the <user.home>/.jreportrc file. You can only edit the file to change the reporthome after the Report Server initialization.
...
Report Server creates the instanceID during its first initializing. It is a string of HEX encoded hash value. For example:
...
If Report Server cannot get the reporthome from CustomizedServerEnv, it will create a default reporthome in <user.home>/.jreport/default.
...
- Since different Report instances cannot access the same Report Server system database at the same time, you will need to create a dbconfig.xml file to store the connection information, and put it in jrenv.jar in workspace\bin (you can find the jrenv.jar file after extracting jreport.war) for each Report Server WAR/EAR.
    For more information about modifying dbconfig.xml, see Configuring Server Databases for Server Metadata.

- When the Report Server WAR/EAR has been deployed, Report will create a file .jreportrc in <user.home>, and each Report instance will read its reporthome from this file. This file must be created by Report Server; however, you can edit it in order to change the reporthome after it is created.
...
<!-- ReportServer calls my.ReportServerEnv to obtain reporthome and server properties.-->
<env-entry>
