---
title: "Exporting Reports to Text"
id: 45190617920269
section: "Delivering Your Reports - Report Designer"
category: "Logi Report"
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45190617920269-Exporting-Reports-to-Text
updated_at: 2026-04-30T15:15:09Z
...
- Navigate to File > Export > To Text. Designer displays the Export to Text dialog box.
    

- By default, Designer saves the Text file in the same folder and same name as the report file. You can type the destination directory and file name in the Export to text box, or select Browse to specify the location and file name with File Explorer of your operating system. When the file name you specify does not contain the .txt extension, Designer automatically adds the extension to the output file. If you want to use your preferred extension (or to have no extension) for the file name of the output, select Use Custom File Extension, then you can type your file name with any extension or no extension.
...
- Make sure Delimited Format is cleared.

- Specify whether to use user-defined density to export the report. If yes, specify the horizontal density and vertical density respectively (select here for some notes about setting the density). 
- 
Horizontal Density
...
- By default, Designer exports all headers and footers in the report, including Report Header/Footer, Page Header/Footer, and Group Header/Footer. If you only want to export data in the Detail panel, clear Header and Footer.

- Specify whether to use CR-LF in Windows convention, or LF in UNIX convention as the end-of-line character.

- Select Run Linked Report if you want to generate the reports that you link with the report (not including the detail reports) in the Text output. If you are only interested in the primary report, leave this option cleared. Generating linked reports at the same time, especially when the linked reports contain a large amount of data, causes performance issue.
...
- In the Report Inspector, select the node that represents the report tab and set the Columned property to false.

- Navigate to File > Export > To Text.

- In the Export to Text dialog box, keep the default export destination.

- Select Delimited Format.

- Select CSV Format from the Format drop-down list.
  

- To well align the records in the CSV output, you should select Use Quote Mark; otherwise, the commas in data values such as $78,321.00 conflicts with the delimiter (comma), and may potentially cause the data being separated incorrectly when the output file is further processed by other applications.     For example, in the following output, it  is difficult to figure out whether the value is $78321.00 or $78 and 321.00.

Scarsdale,The Java King,(914) 657-9823,(914) 657-8745,$78,321.00

- Select Repeat Last Column Value If Null so that when a cell in the CSV output has no value, Designer uses the value of the previous cell in the same column.

- By default, Designer removes the blank spaces at the beginning and end of the field values in the CSV output. If you want to keep the spaces, clear Trim Blank Spaces.

- Keep the other settings to the defaults. 
...
- In Designer, open the page report tab you want to run to the delimited format on Server.

- In the Report Inspector, select the node of the report tab, set the properties Fast Pass and Columned to true.

- Choose the object you want to display in the output, and then set the location for where you want to display it in the output.
...
- Go to the server.properties file in <server_install_root>\bin, and change the property engine.single_thread to false. This enables the property Fast Pass to take effect when running the report tab to the delimited format.

- Publish the page report containing the report tab to Server. When you run or schedule the report, after you choose the report tab and then select the Text format, you can select Delimited Format to generate a delimited format file.

 When you set the Fast Pass property to "true", in the banded header panel, the values of the objects that use global parameters to make calculations are incorrect. Page level calculation, such as Page Number and Total Page Number, is also incorrect. In order to avoid such errors, you should move these objects from the banded header panel to the banded footer panel.
