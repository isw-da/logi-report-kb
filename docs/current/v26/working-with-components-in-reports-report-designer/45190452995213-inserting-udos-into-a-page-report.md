...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45190452995213-Inserting-UDOs-into-a-Page-Report
updated_at: 2026-04-30T15:12:28Z
...
This topic describes how you can insert UDOs in a page report. It also presents an example to show how you can insert a quadrant chart into a banded object using a UDO.
 ...
 You can insert UDOs  in banded panels in query-based page reports only.

- Make sure you have modified setenv.bat  in <install_root>\bin to include the required classes.

- Start Designer.

- Create a new page report, or open an existing page report. The report should contain a banded object created using a query resource.

- Do one of the following:
    
- From the Components panel, drag the UDO icon   in the Others category to the banded panel where you want to insert the UDO. 
...
- The UDO drop-down list contains all the UDOs that you have registered in setenv.bat.
...
Report provides you with a demo UDO, with which you can insert a quadrant chart into a banded object. This demo UDO requires four JAR files: myudo.jar, jfreechart-1.0.13.jar, jfreechart-1.0.13-swt.jar, and jcommon-1.0.16.jar. You can get myudo.jar in <install_root>\help\samples\UDODemo\JQuadrantChart, and you can further edit it if you want. You can download the rest three JAR files from the website: www.jfree.org/jfreechart.

To insert a quadrant chart into a banded object using the demo UDO

- Append the four JAR files to the ADDCLASSPATH variable of the setenv.bat batch file in <install_root>\bin.

- Add the following to the file udo.ini in <install_root>\lib:
Begin
...
- In the Open Catalog File dialog box, browse to open the catalog file SampleReports.cat in <install_root>\Demo\Reports\SampleReports.
...
- In the Data panel, select <New Summary...> from the Summaries node.

- In the New Summary dialog box, select Sum from the Aggregate Function drop-down list, add the field Price from the Products table to the Summary On text box and Product ID to the Group By text box, and then select OK.
    

- Type Sum Price in the Summary Name dialog box and select OK to create the summary.

- Repeat steps 7 to 9 to create another summary Sum Cost, which uses Sum as the aggregate function, summarizes on the field Cost, and is grouped by Product ID. 

- Resize the group header panel of the banded object and add the two summaries to the panel.

- Select the banded page header panel (BPH) and select Insert > UDO.

- In the Insert UDO dialog box, select JQuadrantChart from the UDO drop-down list, and then select OK.

- Select in the banded page header panel to insert the UDO there.

- Keep JQuadrantChart focused and go to the Report Inspector to define its properties.
- 
...
- Resize the banded page header panel and save the report.
