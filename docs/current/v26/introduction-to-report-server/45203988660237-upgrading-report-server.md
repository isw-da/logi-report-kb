...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45203988660237-Upgrading-Report-Server
...
Server provides two migration tools in the <install_root>\bin folder, to convert the resources on the previous version of Server before version 6.0. Resources that the migration tools cover include the security information (realm, user, group, protection, and ACL), report resources (catalog and reports), scheduled tables, completed tables, versions, and version tables, and other relevant information (such as fonts, NLS, and style groups). If you are upgrading from a release later than v6.0, you do not need these tools. 
 ...
 When installing Server v26 into the same directory as the old version, Server maintains the report-level resources in the old version  and does not replace them with the report resources of v26. For example, if you install Server v26 to  C:\Serverv13, Server maintains the backup files of v13   in C:\Serverv13\bak\previous_version, the removed patches and jar files in C:\Serverv13\bak\xxx\lib, and the whole bin folder in C:\Serverv13\bak\xxx\bin.

The following sections show how to upgrade Server from a specific old version to v26.

Upgrading from v13 and later to v26

This section describes  how to upgrade Server v13 to v26. Assume that the old  server is in C:\Serverv13.

-  Install Server v26 to C:\Serverv13. 
...
Upgrading from v6.0 and later to v26

This section describes how to upgrade Server v11.1 to v26. Assume that the old server is in C:\Serverv11.1.

- Check the property values in dbconfig.xml in C:\Serverv11.1\bin or back up the file first.

-  Install Server v26 to C:\Serverv11.1. 
...
Parameter- orgReporthome
          The report home of the original Server. If you do not provide this parameter, Server uses the report home of Server v26 as its value.

- 
Case 1: Installing Server v26 to a new folder (recommended)
- Assuming the old Server is in C:\Serverv595, install Server v26 to a new folder C:\Serverv26. DO NOT start the newly installed Server.

- In the DOS window, switch to <install_root>\bin and run MigrationV52 C:\Serverv595.

- 
Case 2: Installing Server v26 to the folder where the old version resides
- Assuming the old Server is in C:\Serverv595, install Server v26 to the same location. DO NOT start the newly installed Server.

- In the DOS window, switch to <install_root>\bin and run MigrationV52.bat.

Upgrading a version earlier than v5.2 Build 590 to v26

You can do the upgrade using the migration tool MigrationBV52.bat (MigrationBV52.sh for UNIX) in the <install_root>\bin folder. You use this tool to convert the resources of versions lower than v5.2 Build 590 to the resources of Server v26. If you install the new version to the same folder as the old one, you can omit the orgReporthome parameter.
...
- 
Parameter- orgReporthome 
          The report home of the original Server. If you do not provide the parameter, Server uses the report home of Server v26 as its value.

- 
Case 1: Installing Server v26 to a new folder (recommended)
- Assuming the old Server is in C:\Serverv580, install Server v26 to a new folder C:\Serverv26. DO NOT start the newly installed Server.

- In the DOS window, switch to <install_root>\bin and run MigrationBV52 C:\Serverv580.

- 
Case 2: Installing Server v26 to the folder where the old version resides
- Assuming the old Server is in C:\Serverv580, install Server v26 to the same location. DO NOT start the newly installed Server.
