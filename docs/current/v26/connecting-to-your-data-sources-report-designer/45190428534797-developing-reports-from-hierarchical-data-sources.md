...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45190428534797-Developing-Reports-from-Hierarchical-Data-Sources
updated_at: 2026-04-30T15:12:33Z
...
- In the Catalog Manager, expand the data source to which to add the HDS.

- Right-click the Parameters node and select New Parameter from the shortcut menu.

- In the New Parameter dialog box, specify a name for the parameter, choose String as the value type, and specify the URI of the XML file from which to import the HDS as the prompt value (for more information about creating parameters, see Creating a Parameter).

- Right-click the data source node in the Catalog Manager, and then select New XML Hierarchical Data Source.

- In the New XML Hierarchical Data Source dialog box, type @ParameterName in the XML URI box. For example, if you named the parameter in step 2 as "XML_URI", here you should type @XML_URI. In the XSD URI box, specify the URI of the corresponding XSD file. Select OK to add the HDS. 

- 
Create a page report with the imported HDS. When you view the report, specify the URI of the XML file which matches the XSD file in the parameter. The report then shows the corresponding data.

You can also use another way to develop report with dynamic XML URI as follows:

- In the Catalog Manager, import an XML HDS with an XSD file, and then create a report with the imported HDS.

- Create a parameter as described in the previous procedure.

- In the Catalog Manager, go to the node of the imported HDS, expand the Properties sheet, and then change the property URI to @ParameterName.
...
- 
Leaf
    A leaf in parallel HDS refers to a column (DBField), nodes that contain no subnodes.

- 
Branch
    A branch in parallel HDS is a special table that contains leaves or subbranches.
    

- 
Parallel branches
    When the content of two branches has no parental relationship, these branches are parallel branches.
...
- When you remove a branch using the component wizard, Designer also removes the related panel.
...
- In the Catalog Manager, select the HDS that is used by the report from the Hierarchical node, and then expand the Properties sheet.
...
- Navigate to File > Publish > Publish Report to Server to publish the folder (for more information, see Publishing Resources Remotely).

- Start  Server. You are now able to run the report on it.
