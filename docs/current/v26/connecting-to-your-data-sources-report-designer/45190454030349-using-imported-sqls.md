...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45190454030349-Using-Imported-SQLs
...
When you write the SQL statement, you can use parameters and constant level formulas predefined in the catalog data source in which you have created the JDBC connection in the format @FieldName or :FieldName to calculate your data. For example, if you need to get different result sets from the SQL statement at runtime, you can reference parameters  in the WHERE clause of the SQL statement to dynamically filter the data. In the preceding SQL example, if you want to use a parameter to return a result set in which the customer ID is greater than the parameter, then the WHERE clause would be like this:
...
However, if a parameter allows for multiple values and is enabled to use one single value "All" to represent all its values (the parameter's Enable the "All Values" Option is "true"), to use the parameter in the SQL statement, you should embed the IN condition in parenthesis to enable the "All" value to work properly, for example: 
...
Besides using parameters, you can also use the User Name special field as @username in the WHERE clause of the SQL statement to filter the data.
...
The imported SQLs can work the same as queries in Designer. For example, you can use imported SQLs to create page reports directly, use the Data Manager to control data retrieval of the imported SQLs, and create cached result files for the imported SQLs. However, there are still some differences between queries and imported SQLs.
...
You can also use imported SQLs to build queries and business views.
...
- In the Catalog Manager resource tree, right-click the JDBC connection and select Add SQL on the shortcut menu. Designer displays the Imported SQL Editor dialog box.

- In the SQL Name text box, type a name for the imported SQL. You should provide a unique name that has not been used in the catalog; otherwise, Designer prompts you a warning message when you save the imported SQL.
...
- Create the statement of the imported SQL. You can write the SQL statement directly in the text panel or paste it from an outside editor. To import the SQL statement from an SQL file, select Import SQL, then in the Select an SQL File dialog box, browse to the SQL file and select Open. Designer loads the SQL statement in the file into the Imported SQL Editor dialog box.

 When you import the SQL statement from an SQL file,
...
- If the database catalog selected in the From DB Catalog drop-down list does not contain the tables/views/synonyms referenced in the SQL statement, Designer displays an error message when you select  to check the statement syntax. In this case, you can reselect the catalog from the drop-down list in accordance with the actual database catalog of the SQL statement. There is also another way to avoid such error: you can use 3-part names in the SQL file, for example, SELECT catalog.schema.t.c from catalog.schema.t, then no matter what database catalog you have selected in the From DB Catalog drop-down list, the SQL statement imported from an outside SQL file can always work correctly in Designer.

- To export the SQL statement to an SQL file, select Export SQL, then in the Save SQL dialog box, provide the location and name of the SQL file and select Save.

- Select Save & Exit to save the imported SQL into the catalog and close the editor.

- Designer adds the imported SQL to the Imported SQLs node in the catalog resource tree. You can right-click it and select Show SQL from the shortcut menu to view its SQL statement.
...
- From the Imported SQLs node in the catalog resource tree, select the imported SQL, right-click it and select Update on the shortcut menu.

- In the Imported SQL Editor dialog box, work with the SQL statement as described in the previous procedure, except that you cannot change the SQL Name.
