...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45204046149389-Major-Benefits-of-Web-Reports
...
Web reports also support agile development techniques such as continuous integration, by allowing you to update report templates by both Web Report Studio and Report Designer.

- 
Fewer functions
    Web reports that use .wls as the file suffix, support a subset of functions of Report page reports that use .cls as the file suffix. The basic and essential functions not only guarantee a good report presentation, but also make the report design experience easier for a new user of Report. 
...
Fast report rendering
    At runtime, Web Report Studio provides much higher performance when viewed from a browser compared to viewing a page report using Page Report Studio. Page Report Studio must send all the user action requests to Report Server which renders the new page on the server and updates the browser view. Web Report Studio does many of the actions locally on the client in the browser, because they require only a change in rendering the view. Report Engine enables as much of the processing as possible to complete on the client side, allowing much higher scalability for Report Server to handle more simultaneous users.

- 
Creation and modification in both Report Server and Designer
 
  After you create web reports using the Web Report Wizard, you can download them from Report Server and edit them in Report Designer. You can also create web reports in Report Designer and then publish them to Server just like page reports.
