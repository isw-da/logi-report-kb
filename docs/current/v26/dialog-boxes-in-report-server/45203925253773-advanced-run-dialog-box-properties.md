...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45203925253773-Advanced-Run-Dialog-Box-Properties
updated_at: 2026-07-30T20:49:01Z
source_host: logi-report-v26.insightsoftware.com
---
# 
Advanced Run Dialog Box Properties

Use the Advanced Run dialog box to run a report in the Advanced mode. This topic describes how you can specify advanced settings for running a report.
...
- 
Catalog
Catalog name and path in the server resource tree. Unavailable to shared reports.

Select Select Another Catalog if you want to choose another catalog for the report in the Select Another Catalog dialog box.
...
- 
Enable Style Group
If you have specified a default style group via the Override Style Group property in the server profile, Server selects this property and displays the default style group in the Style Group box by default. You can also choose another style group from the Style Group list for the selected page report tabs or web report.

When the <No Style> item displays in the Override Style Group box, Server clears this property by default and uses the style group property of the page report tab or web report for this format that you predefined in Report Designer.
...
Select to apply the default on-screen filter values that you have specified to the report. Server displays the property when you did not clear Enable Setting Default On-screen Filter Values For the corresponding report type in the server profile.
...
Select to save the report result version to the built-in version folder. Server does not display the property to organization users when the report is in the Public Reports folder, and to shared reports either.
...
Select to enable multiple versions for the report result. Then, type the maximum number of versions that the report result can have. The default value 0 means unlimited version number.
...
Available only when the Archive Location is a public folder and when you have the Grant permission on the report. Select Set Permissions to set user permissions on the report in the Set Permissions dialog box.

## 
Duration Tab Properties

Server displays the Duration tab only when you have enabled the task-level timeout mechanism. In this tab, you can specify a time duration for a task, and ask Server to cancel the task or to notify you or someone else of the task status via email if the task has not finished running when the task duration is up.
...
- 
Edit Mode
Select if you want Web Report Studio to run in the Edit Mode by default. You can switch to View Mode if it is available. This item is available only when you have the Edit permission on the report.
...
- 
Interactive View
Select if you want Page Report Studio to run in the Interactive View by default. You can switch to Basic View if it is available. This item is available only when you have the Edit permission on the report.

Profile

Select the Page Report Studio profile you want to apply to run the report, which contains a set of Page Report Studio settings.

Background Mode Timeout

Select and then specify the time after which Server will continue to run the report in the background. When it runs a page report and has not generated the result yet after the specified time, Server will automatically run the report in the background mode. Once the report is complete you can find the result in the My Tasks > Background Tasks table.

This property takes effect when you have selected Enable Background Task for Reports in the server profile.
...
Select if you want the output file to have no file extension or want to use a different file extension from the default one (.html).
...
Select if you want to generate the report result to multiple HTML files. Server designates a serial number for each HTML page. For example, if you name a 3-page report as "sales", Server will create three files called sales_1.html, sales_2.html, and sales_3.html.
...
Select if you want to encrypt the Excel output. Server displays the Excel Encrypt dialog box for you to configure the Excel encryption settings.
...
Select if you want the output file to have no file extension or want to use a different file extension from the default one (.pdf).
...
Select if you want to encrypt the PDF output. Server displays the Encrypt dialog box for you to configure the encrypt settings.

Sign

Select if you want to add the digital sign to the PDF output. Server displays the Sign dialog box for you to configure the sign settings.
...
Select to make the PDF output to support the PDF/A-3 standard.

Accessible PDF

Select to generate the report result as an accessible PDF file. You can then select Settings and specify the Tag Order property. For more information, select Making HTML and PDF Report Results and Server Console Accessible.

- 
Tag Order
Select Template Object Order if you want the objects to use the order in the Inspector tree.

Select Page Coordinate Order if you want the order of the objects to be determined by the coordinates of the content objects on the report page: The tag corresponding to the content object with a smaller y value will be in the front, and if two content objects have the same y value, the tag corresponding to the content object with a smaller x value will be in the front. If the difference between the y values of two content objects is less than 40 (0.5 point), it should be considered that these two content objects have the same y value.
...
- 
Fit Width
The report contents in a page fill the page width within the window.
...
- 
Fit Visible
The report contents in a page fill the width of its bounding box within the window.

Title

Specify the text you want to use for the Title property of the PDF output.

Subject

Specify the text you want to use for the Subject property of the PDF output.

- If you want to use the float ascent value in the PDF font for embedded TTFs in your PDF output (by default, Server applies that of the JDK font), you can add the -Dlogireport.pdf.UsePdfFontAscent argument to Server's startup file JRServer.bat/JRServer.sh in <install_root>\lib. The value of this argument can contain multiple items separated by a semicolon (";"). Each item should be a TTF font name starting with "*" (for example, -Dlogireport.pdf.UsePdfFontAscent="*Time-RomanBold;*SourceSansPro-Regular") or be the AllEmbeddedTTF keyword (for example, -Dlogireport.pdf.UsePdfFontAscent="AllEmbeddedTTF"). When one of these items is AllEmbeddedTTF, Server applies the ascent of the PDF font for all embedded TTFs in the PDF output; otherwise, Server only applies the ascent of the PDF font for the embedded TTFs the names of which you have included in this argument.

- If you want to map fonts to TTF, add the -Dlogireport.pdf.MapToTTF argument in the server startup file JRServer.bat/JRServer.sh. The value of this argument can contain multiple items separated by a semicolon (";"). Each item should be either the All keyword or a non-TTF font that does not start with the "*" character for the Font Face property. When one of these items is "All", Server maps all non-TTF fonts in the report template to TTF (the Font Face property value starts with the "*" character). That is, if there is a matched TTF file in the font directory of Server installation directory, Server maps the non TTF fonts to the TTF. When none of the value items is "All", for each item used in the report template, Server maps it to TTF.

- To specify which TTF you want to embed into the PDF output, you can add the -Dlogireport.pdf.EmbeddedFonts argument in the server startup file. The value of this argument can contain multiple items separated by a semicolon (";"). Each item should be either the "All" keyword or a TTF that starts with the "*" character for the Font Face property. When one of these items is "All", Server embeds every TTFs in the report template into the PDF output. If none of the value items is "All", for each item used in the report template, Server embeds it into the PDF output.
...
Select if you want the output file to have no file extension or want to use a different file extension from the default one (.xls or .xlsx according to the Excel Version you selected).
...
- 
Column Format
Select if you want to calculate all components' row/column values in the report using the Columned property value of the report when exporting. Choose this format if you want to actively use the report in Excel such as filtering and sorting.
...
Specify whether to overflow the content of the current cell to the next cell in Excel when the next cell is absolutely empty and both cells are text format, non merged, and have no wrapping text. The default value of this option is true.

This property does not support Data Format.
...
Select if you want both the cell display value and focused value to use the formatted value. Otherwise, only the cell display value will use the formatted value, and the focused value will be the original data.

This property does not support Data Format.

- 
Generate Excel Formula
Specify whether to generate the Excel formula for summary fields if the corresponding conditions are met. Otherwise, the Excel formula will not be generated. The default value of this option is true.
...
Select if you want the output file to have no file extension or want to use a different file extension from the default one (.txt, .dat, or .csv according to the File Suffix property value).
...
Select if you want to remove the blank spaces at the beginning and end of field values, instead of keeping them.
 ...
 When exporting to CSV, content in any cell that begins with one of the formula triggering characters =, -, +, or @ will be prefixed with a tab character (note that if there are quotes, the tab character goes inside of the quotes) since v23.4. If you don't want to add the prefix, clear Escape Formula for CSV on the Administration > Configuration > Export > Text tab.

Horizontal Density

Type an integer value for each unit of the horizontal density between the columns. The resulting density is a direct ratio of the value you specify. That is, the greater the value, the smaller the width between the columns. By default, Report defines the density.

Vertical Density

Type an integer value for each unit of the vertical density between the columns. The resulting density is a direct ratio of the value you specify. That is, the greater the value, the smaller the height between the columns. By default, Report defines the density.
...
- The value of Horizontal/Vertical Density must be greater than the character's width/height of the smallest field in the report (smallest field is the field with the smallest font size), otherwise, Report does not apply the value you set.

- If one of the values of Horizontal/Vertical Density is greater than 0 and the other one is less than 0, Report applies the value greater than 0 and defines the other value.

- If the values of Vertical Density and Horizontal Density are both greater than 0 and the value of Horizontal Density is less than 1, Report applies the specified value of the two densities. Otherwise, Report defines the two values.

- If the values of Vertical Density and Horizontal Density are both less than 1, Report defines the two values.
...
Select to use Windows end-of-line characters to indicate the start of a new line. Report will use two characters <cr> and <lf> at the end of the line.

Unix End-of-line (LF)

Select to use UNIX End-of-line characters to indicate the start of a new line. Report will only use the UNIX character <lf> at the end of the line.
...
Specify the suffix of the output file. When you clear Delimited Format, the output file suffix can be .txt or .dat. When you select Delimited Format, the output file suffix is .csv.

This property is available in the Schedule dialog box.
...
Select if you want the output file to have no file extension or want to use a different file extension from the default one (.rtf).
...
Select if you want the output file to have no file extension or want to use a different file extension from the default one (.xml).
...
Select if you want the output file to have no file extension or want to use a different file extension from the default one (.ps).
