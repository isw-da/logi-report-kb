...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45190604293901-Configuring-the-Logging-System
updated_at: 2026-04-30T15:15:14Z
source_host: logi-report-v26.insightsoftware.com
---
# Configuring the Logging System

Report provides a robust and flexible logging system that implements the SLF4J (Simple Logging Facade for Java) solution, enabling you to plug in the logging framework you want to use easily. This topic introduces the default Report logging system that is based on Apache Log4j 2 and how you can configure it to serve your requirements. It also describes how you can use SLF4J with different logging frameworks in Designer.
...
To configure the behavior of Report's default logging system, perform either of the following:
...
The default logging system of Report provides certain appenders to which you can output the log content. By default, the output file is in the 
...
Report's default logging system supports the following log levels, which are sorted according to the amount of the logged information, from the least to the most.
...
You can configure the behavior of SimpleLogger by creating the simplelogger.properties configuration file and adding this file to the slf4j-simple library. For more information about SimpleLogger, go to www.slf4j.org/api/org/slf4j/simple/SimpleLogger.html. If you do not add the configuration file, Designer outputs the log to LogiReport.out.log in <designer_install_root>\logs by default, and you can use the -Dorg.slf4j.simpleLogger.defaultLogLevel option at command-line startup to specify the log level to be trace, debug, info, warn, error, or off.
...
For more information about JUL, go to docs.oracle.com/javase/8/docs/technotes/guides/logging/index.html.
...
For more information about Logevents, go to github.com/jhannes/logevents/blob/main/MANUAL.md.
...
You can configure the behavior of Log4j by creating a configuration file, and then using the -Dlog4j.configuration option to add this file to the CLASSPATH environment variable of Designer's startup file JReport.bat in  <designer_install_root>\bin, for example:
...
For more information about Log4j, go to logging.apache.org/log4j/1.2/manual.html.
...
You can perform log configuration for reload4j using the same method as you do for Log4j. For more information about reload4j, go to reload4j.qos.ch/.
...
You can configure the behavior of logback by creating a configuration file, and then using the -Dlogback.configurationFile option to add this file to the CLASSPATH environment variable of Designer's startup file JReport.bat in  <designer_install_root>\bin, for example:
...
...-classpath "%CLASSPATH%" -Dreporthome="%REPORTHOME%" -Dlogback.configurationFile="file:\C:\LogiReport\Designer\logback.xml" -Djreport.url.encoding="UTF-8"...
 ...
 For more information about logback, go to logback.qos.ch/documentation.html.
