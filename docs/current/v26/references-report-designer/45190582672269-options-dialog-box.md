...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45190582672269-Options-Dialog-Box
updated_at: 2026-04-30T15:14:04Z
...
Show warning message when CSS properties are not supported by Report

Select to show the warning message when the CSS style you apply contains properties that Report does not support.
...
Specify the user name which Designer applies in cases when a user name value is required, for example, when you run a report which references the User Name special field.
...
You can also customize the ratio by adding the -Dsun.java2d.uiScale option to the CLASSPATH environment variable of Designer's startup file JReport.bat in <install_root>\bin. Valid values for the option are: 1, 1.25, 1.5, 1.75, 2, 2.25, and 2.5. 
...
Use this category to specify properties of the report editor, that is, the Design tab in the main window. 
...
- 
Margin Line Color
Select the color of the margin line. You can also select Custom from the drop-down list to customize a color in the Pick a Color dialog box.
...
- 
Color
Select the color of the grids. You can also select Custom from the drop-down list to customize a color in the Pick a Color dialog box.

Display Attributes

You can specify the display attributes for Designer in this box, which you can also set in the Display Attributes dialog box.
...
- 
Foreground
Select the foreground color of the text in the columns. You can also select Custom from the drop-down list to customize a color in the Pick a Color dialog box.
...
- 
Border Color
Select the color of the borders. You can also select Custom from the drop-down list to customize a color in the Pick a Color dialog box.
...
Select the background color of the columns. You can also select Custom from the drop-down list to customize a color in the Pick a Color dialog box.
...
Specify the space between the text and the right border of the column.
 ...
 Designer applies the padding values  when you add data columns to a catalog. You can adjust the values for each column in the Catalog Manager, or after adding the columns to reports in the Report Inspector.
...
Use this category to specify options for previewing reports in Designer.
...
Select if you want Designer to check for differences which can cause Report Engine to fail in running a report, Designer then displays all the resources referenced by the report in the Merge dialog box and marks the critical conflicting resources only. When you select this option, Designer may return different values when you run the saved report later.
...
Select to use cached query result instead of the database to retrieve data for running reports. If you select this option, when you view or run a report that uses a query or business view with cached query result, Designer displays a dialog box for you to choose the result file with which to get data for the report.
...
Designer displays options of this category in four tabs: HTML, E-mail,Fax, and Text.
...
Select to open the Advanced Export Settings dialog box to customize the layout precision for each export format.

### 
HTML Tab

You can specify preferences for exporting the report to HTML in this tab.
...
- 
Multiple files
Select to export the report into multiple files and designate a serial number for each HTML file. For example, if you named a 3-page report as Sales, Designer creates three files: Sales_1.html, Sales_2.html, and Sales_3.html.
...
If you select this option, Designer exports the accessibility attributes that you define for the report elements in the Report Inspector to HTML  which is Section 508 compliant. For more information, see Exporting Reports to Accessible HTML and PDF.
...
You can specify the email server and preferences for exporting the report to email in this tab.
...
You can specify fax settings for exporting the report to fax in this tab. You can export the report either via a fax machine or a fax server.
...
Select to allow Designer to add a tab character in the front of the content in any cell in your CSV output when the content begins with one of the following characters: "=", "+", "-", and "@". 
...
Use this category to specify options for printing reports. 
...
Use this category to specify the default properties of the Query Editor that you use for creating queries. 
...
Select how you want to cache data of cascading parameters.

- 
False/True
Specify the default cache setting for cascading parameters at parameter level.
- When you select False, if the cache setting for a cascading group is set to Default, Designer does not cache data of parameters in the group.

- When you select True, if the cache setting for a cascading group is set to Default, Designer caches data of parameters in the group.

- When you select either False or True, if the cache setting for a cascading group is not set to Default, Designer applies the  parameter level cache setting.

- 
Cache None/Cache All
Specify the default cache setting for cascading parameters at application level. When you select either option, it overrides the cache setting at parameter level.- Select Cache None to prevent Designer from caching data of any cascading parameters. 

- Select Cache All to let Designer cache data of all cascading parameters.
...
Use this category to specify options for connecting with a started Report Server to publish or get resources from the Server.
...
When both the connection and user information is remembered by Designer and the specified Server is started, when you want to connect to the Server to publish or get resources, Designer sets up the connection and signs you in automatically; otherwise, Designer displays the Connect to Report Server dialog box for you to specify the connection and user information.
