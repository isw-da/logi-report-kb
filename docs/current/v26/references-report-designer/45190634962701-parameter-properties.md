...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45190634962701-Parameter-Properties
updated_at: 2026-04-30T15:15:30Z
source_host: logi-report-v26.insightsoftware.com
---
# 
Parameter Properties

This topic describes the properties of a Parameter object in a catalog.
...
| Get Value from API Only | Specifies whether the parameter values can only be specified using API. When this property is "true", users should obtain parameter values only from API and there is no parameter dialog box available in any Designer or Server UI for inputting values. Users need to provide values of the parameter by API such as URLs, coding, and sessions; otherwise, Report Engine applies the default value of the parameter. When this property is "false", the parameter values can be obtained from both GUI and API. When a member of a cascading parameter group can get values from API only, it means that all the higher level members shall get values from API only. This also applies to parameters on which other parameters depend. When several parameters share values in JDashboard, if any of them can get values from API only, they will share the values from API. Data type: Boolean |
...
| Use Current Date-Time | Specifies whether to use the current system's date time as value of the parameter when the parameter is Date, Time, or DateTime data type. Data type: Boolean |
| Use Current Date-Time When Blank | Specifies whether to use the current system's date time as value of the parameter when the parameter is Date, Time, or DateTime data type, and the parameter value is blank. Data type: Boolean |
...
| Value List | Shows values of the parameter. Read only. |
...
| Auto Scale in Number | Designer displays this property when the object is Number data type. You can use it to specify whether to automatically scale values of the object that fall into the two ranges: When 1000 = 10^15, Designer uses scientific notation to scale the values. The option "auto" means that the property setting follows that of the parent data component when you use the object in a report. Data type: Boolean |
...
| Others |  |
| Data Mapping File | Specifies the data mapping file (without the locale part) you want to apply to the parameter for NLS use. For example, if the data mapping file is Product_de_DE.properties, type Product in the value cell. When you select , if the parameter is bound with a single column or cascading columns, Report Engine applies the data mapping file that you specify for its display column to the parameter; otherwise, it does not apply any data mapping file to the parameter. Data type: String |
