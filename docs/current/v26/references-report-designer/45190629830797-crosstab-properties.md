...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45190629830797-Crosstab-Properties
updated_at: 2026-07-30T22:56:21Z
...
| Geometry |  |  |
| Height | Page Report, Web Report, Library Component | Displays the height of the object. Read only. |
| Width | Page Report, Web Report, Library Component | Displays the width of the object. Read only. |
...
| Pattern List | Page Report, Web Report, Library Component | Specifies the patterns for the alternating color you want to apply to the crosstab. Select the ellipsis in the value cell to specify the color patterns in the Color List dialog box. Data type: String |
| Show | Page Report, Web Report, Library Component | Specifies whether to apply alternating color in the crosstab. Data type: Boolean To apply alternating color in a crosstab, the aggregate fields in the crosstab should have transparent background. |
| CSS |  |  |
...
| Enable Navigation | Web Report | Specifies whether to display scroll bars on the crosstab at runtime, when the parent container cannot fully display its data. When you set this property to "true", you can further specify the height and width of the frame for holding the crosstab via Navigation Height and Navigation Width. Data type: Boolean |
...
| Crosstab Property |  |  |
| Avoid Orphan Header | Query Page Report | Specifies whether to display the column headers together with the data. When you set this property to "true", if the column headers happen to be at the bottom of a page, Designer displays them together with the data on the next page. Data type: Boolean |
| Block Gap | Query Page Report, Web Report | Specifies the space between each part when the crosstab is split into more than one part. If the value is negative, the new split part will start on a new page. Type a numeric value to change the gap.Data type: Float |
...
| Expand Data | Query Page Report | Specifies whether to enable Page Report Studio users to expand/collapse the column/row groups in the crosstab. Data type: Boolean |
...
| Repeat Aggregate | Query Page Report | Specifies whether to repeat the crosstab for different aggregate fields. Data type: Boolean |
| Repeat dimension Header | Page Report | Specifies whether to display the dimension headers on every page when the crosstab spans more than one page. Data type: Boolean |
...
| Vertical Layout | Page Report, Web Report, Library Component | Shows whether the aggregate fields display vertically in the crosstab. Read only. |
| Split in Z-Direction | Page Report | In addition to specifying the direction in which a large crosstab is split (wrapped) across multiple pages for paginated outputs, it can also control the calculation direction of running aggregation. Data type: Boolean |
| TOC |  |  |
...
| TOC Anchor | Page Report, Web Report | Specifies whether to include the object in the TOC of the report. Data type: Boolean Data type: Boolean |
| Accessibility |  |  |
...
- 
Expand Detail Data: A property on DBFields and formulas in crosstab as column/row fields. You can use it to specify whether to expand details of specific column/row groups in a crosstab by default.

 These two properties work only in continuous page mode and do not apply to the innermost column/row group. After setting the two properties, if you further modify the crosstab layout, they may not take effect.
...
- Clear Page Layout in the View ribbon to switch to continuous page mode.

- Publish the crosstab to Server and run it in Page Report Studio.
