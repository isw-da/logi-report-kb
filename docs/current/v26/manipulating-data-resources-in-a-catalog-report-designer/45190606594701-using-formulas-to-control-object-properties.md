...
url: https://logi-report-v26.insightsoftware.com/hc/en-us/articles/45190606594701-Using-Formulas-to-Control-Object-Properties
updated_at: 2026-04-30T15:15:11Z
...
While editing the property values of an object in the Report Inspector, when you select some properties, you may notice that Designer displays the formula button  in the value cell (Designer also displays the button next to the value text boxes in some dialog boxes, for example, the Display Type dialog box and some chart format dialog boxes).  For object properties of this type, you can use formulas to control their values.

 To set a formula as the value of a property, select  and it changes to another button . Select the drop-down list in the value cell and Designer displays a formula list.

You can specify the formula you want to use to return value for the property using one of the following methods:

- Select an existing formula from the drop-down list. For a query-based page report, Designer lists the formulas in the current catalog that are valid for the property; for a business view-based report, you can only select from the valid dynamic formulas in the current report. Designer then displays the name of the formula prefixed by the "@" symbol as the property value.               
...
- When the object is in a crosstab in a page or web report, you can also use a crosstab formula to return value to the property.

However, when you want to use formulas to control properties of the objects whose parent does not relate with a data resource, for example, an object directly contained in the report body, you need to first bind a dataset to the report body.

- Some properties must be known by Report Engine before a page break, so that it can lay out the object correctly. In a formula or expression, the PageNumber special field is the switch for setting the page break calculation time. If   a formula or expression references PageNumber, the calculation time is after the page break; otherwise, the calculation time is before the page break. For most properties, you can control their values by formula or expression at any time without concerning about the page break.
...
- 
Create a page report with a standard banded object in it using a query in Data Source 1 of the catalog. 

- In the design area, select the detail panel (DT) of the banded object.

- In the Properties sheet of the Report Inspector, select the value cell of the Background property, then select  and select MColor from the drop-down list.
...
- Create three formulas.    Continue1:
...
- Insert Continue1 in the banded header panel (BH), Continue2 in the banded footer panel (BF). To track the calculation, you can insert Continue3 in the banded page footer panel (BPF).

- Right-click the banded page header panel (BPH) and select Insert Panel Before to insert another BPH before the existing BPH.
...
- Set the Record Location property of the label to Page Footer. Continue3 references the Page Number special field  so Report Engine executes it as a page level formula in the order the formula is encountered rather than at the end of a group or the banded object. Here, Continue3 returns value which Report Engine calculates in the banded page footer.

- Select the View tab to preview the banded object. The "Continued on Next Page" label displays on every page except the last page.
