...
url: https://docs-report.zendesk.com/hc/en-us/articles/28891700527245-Migrating-Server-and-Server-Data
updated_at: 2026-02-26T02:13:16Z
...
You can migrate a Report Server to another Server on other computers, by duplicating the environment of the Server. The migration procedure is divided into two parts: migrating server system configurations and migrating server data between computers. This section describes the migration based on Windows. It also applies to other systems.

### 
Migrating Server System Configurations

When two Servers have the same license, same Report and JDK versions, and different system databases, you can migrate the system configurations of the original Server on computer A to the target Server on computer B. The server system configurations that you can migrate include JVM class path, JVM heap size, other JVM options, patch, JDBC driver, customized classes (such as UDS, UDF, and UDO), customized JSP, white label, and configurations stored in files like server.properties.
...
- 
Back up the data in the system, realm, and profiling databases of the Server on computer A. 

- Copy the files that you created in step 1 from computer A to computer B.

- 
Restore the data to the Server on computer B.

- Start the Server on computer B. The migration will be completed.

If your server is V16 or above, you can migrate the server data using an easier way. 
...
| -backup: | Back up the server data to a migration file, for example: MigrationTool –backup:d:\tmp\bk.dat |
| -restore: | Restore the server data from a migration file, for example: MigrationTool –restore:d:\tmp\bk.dat . |
...
- Download the latest image of Server from https://hub.docker.com/r/logianalytics/logireport-server/tags. You can find all available versions on the page.
    docker pull logianalytics/logireport-server:latest

- List all the containers on your host. Pick up the previous version Server container name. Here it is server15.6.
    ./docker-container-migration.sh -l

- Start the migration process.  The previous version Server container stops during the process.
