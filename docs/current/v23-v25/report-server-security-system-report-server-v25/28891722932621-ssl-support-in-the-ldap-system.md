...
url: https://docs-report.zendesk.com/hc/en-us/articles/28891722932621-SSL-Support-in-the-LDAP-System
...
Report Server's LDAP system supports SSL when connecting to an LDAP server for obtaining security information. This topic describes the problems that may happen when you use SSL in the LDAP system. 

Solving the wrong connection port type problem 

Report implements a method in the security system to solve the wrong connection port type problem.
...
Since the connection time varies with the user's network environment, it is better to set the socket timeout time in LDAPProperties.xml: modify the value of the element env-socketTime before you start the server. Its default value is 10, which means that the socket timeout time is 10 seconds. You can modify this value according to your network environment.

About SSL certificate store in Report Server 

Since JNDI uses the default SSL provider, the certificates will be checked by JSSE's default TrustManager: X509TrustManager. If the TrustManager does not accept them, Report Server will store the SSL certificates into another key store file. This file is in  <install_root>\properties\LDAPKeyStore.keystore. The password to access the file is jinfonet. You can also use -D parameters to specify another file and password. For instance, if you want to add the certificates into C:\certs\certs.keystore, and use the password test, you should add the following parameters to Report Server's startup file:
