...
url: https://docs-report.zendesk.com/hc/en-us/articles/28891476412685-Tour-of-the-Java-API
updated_at: 2026-02-26T02:11:03Z
source_host: docs-report.zendesk.com
---
# 
Tour of the Java API 

This tour is a high-level survey of the Report Server Java API. This topic describes the set of APIs to help you narrow down the scope when you need to use Server for a particular task.
...
## 
Initializing and Connecting to Report Server

Every program that uses Report Server Java API must start with calls to ensure an instance of the server is running, and then obtain a handle to the server for use in later API calls.
...
Additionally, the two most commonly used methods of the Java API are in this class. These two methods must be called by any program that wants to use Report Server in order to initialize and connect to Report Server. These will still be used by programs that run outside a servlet container.

- 
Method: HttpUtil.initEnv() - Initialize and start the singleton Report Server running on the machine which all programs will access.

- 
Method: HttpUtil.getHttpRptServer() - Connect to the running Report Server and return a handle to the server to use in future API calls.
...
Report Server provides built-in methods for doing this based on a protocol using HTTP Authentication and query parameters to pass in signed-in credentials. The security mechanism includes a framework system that enables developers to write code to co-operate with Report Server's built-in system so that existing applications can provide the security functionality.
...
- 
Method:  HttpUserSessionManager.checkLogin() - Indicate if a validated Report user signed in to the web session. If no user signed in, it tries to validate and sign in a user based on credentials in the current HTTP request. It does not automatically send a response to the browser to perform an HTTP Authentication.
...
- 
Network security:  Using the checkLogin() system for controlling access to the web pages is not an airtight security protocol because signing in credentials in the HTTP Request are in an insecure stream. If this system is used for production outside a firewall, the network should be secure with URLs to the web pages using https (SSL) instead of http. Secure Socket Layer (SSL) can be easily configured in the Report Server Console. 

- 
Single Sign-on (SSO) - Access control to the Report web pages is based on signing in to the Report web session with a username and password. In the standard setup, Report Server will pop up the signing in dialog box on the first attempt to run a Report web page in a session. This should not happen when a signed-in application user does their first attempt to run a Report web page. Report Server provides a framework to allow SSO, to avoid this experience. When you turn on SSO, Report will just ask the application for the current username and not challenge the user.
    
- 
Interface: jet.server.api.http.HttpExternalAuthorized. This Java interface defines a class that developers can write and deploy within Report Server that co-operates with the checkLogin() methods that control access to web pages.
        This Java interface defines a protocol for the application to tell Report Server that a user already signed in to the application's web session so that checkLogin() will sign in that user to Report Server. It also has a protocol for asking the application to handle the case where no user signed in, so that the application can direct the user to the application's signing in workflow. This enables the application to provide the single place where a user signs in to the web application. This also prevents checkLogin() from using the HTTP Authentication process. 

To turn on SSO, compile and deploy the implementation of this Java interface as a jar, and configure Report Server to use it.

- 
Methods:
- HttpExternalAuthorized.getExternalAuthorizedUser() - Return the Report User ID (username) for the user who signed in to the web session.

- HttpExternalAuthorized.handleUnauthorizedRequest() - Build a response to send to the browser redirecting the user to the application signing in page, and return true to tell Report Server to use that response, instead of the HTTP 401 Unauthorized response.
...
Sample code:   APISecurity\SingleSignOn\CustomHttpExternalAuthorized.java - Example implementation of the interface. Read comments in the source for detailed explanations on the application security model it uses.
        This implementation is perfect for deploying using the command line method of registering it with Report Server, or when using a servlet to register it with Report Server by making an API call. Because of limitations in the JSP parser, it is not usable as the class to use when registering it with Report Server using a JSP page to make the API call.
...
Sample code:   APISecurity\SingleSignOn\com\example\MyExternalAuthorized.java - Example implementation of the interface. Read comments in the source for detailed explanations on the application security model it uses.
        This implementation is written as a package with a multi-tier package name. This can be registered with Report Server as the implementation in all possible cases, include from code in a JSP page that calls an API.

- 
Turning on SSO  - Report Server uses SSO when you registered the implementation class of the Java interface with it.           
        
- 
Register in command line
          Set a system property in the command line that starts up your application or Report Server. The property jrs.httpExternalAuthorized should be set to the name of the implementation class. For example, use -Djrs.httpExternalAuthorized=CustomHttpExternalAuthorized when starting the Java JVM for the application containing the JSP pages that call checkLogin().
...
Where to turn on SSO
          You should turn on SSO for the JVM where the JSP pages are running. If the application and Report JSP pages are running on a web server with Report Server installed on a dedicated back end server,          You should turn on SSO in the web server computer. Report handles properly SSO access control to the web pages for the HTTP requests, while access to the back end Report Server will be over RMI, which does not use the checkLogin() or SSO for access control.
...
- 
Authentication -  This Java interface defines a class that developers can write that replaces the built-in user validation system of Report. Even when using SSO, the system will check that the user is valid in Report so it can determine if the credentials identify a registered user. This Java interface is called to check if the user is valid. A custom implementation can use local DBMS data or the application's data to validate the user. Another option rather than to authenticate with Report's built-in system is to authenticate directly to an LDAP or Active Directory service.     
...
- 
Custom Authorization - This Java interface defines a class that developers can implement to replace the built-in Authorization (Authenticator) class. After a user has been validated as a registered Report User, there is still the question if the user can do the operation requested against the target resource. Methods in this Java interface will be called to determine if the user has permission to do a given action against a given report resource. You can implement your own method for making this decision.
...
- 
Add/Modify/Delete Roles/Groups/Users - This built-in package enables an application to build a system to synchronize their internal system for managing user identity with Report Server's DBMS of user identity. By default, users are added to Report Server manually using the Server Console or by synchronizing with an LDAP service. This package provides a third way, enabling you to add the users, groups and roles into Report Server by making API calls so that you can utilize Report Server's built-in authorization, including permissions to resources and row level and column level security. Since users, groups and roles are in the Report Server DBMS developers are sometimes tempted to directly update the DBMS tables but this is highly discouraged. Since a running Report Server caches the security information, the server ignores any updates not using the API  until it restarts.
...
Report Server uses an event framework model that enables the application to provide methods that are called at various points in the processing of requests in Report Server. These include calls to methods before a report is run, after the parameters are entered by the user to validate the parameters or generate new derived parameters, and after a report is run to log information. Using these methods, you can keep detailed logs of information about the report in addition to the logging information that Report provides.
...
Report Engine enables you to run reports directly in your application without connecting to a Report Server instance. As long as you do not need the Report Server features such as Page Report Studio, security and scheduling then this is an efficient way to embed reporting into your application.
...
- runReport - Run the report and produce a Report rst file that you can use to export to a user viewable report.
...
This Java interface defines an implementation you can build to provide your own methods to do data mapping and text replacement. The implementation that is provided by Report Server works out of the box, but can be changed easily.
...
This RptServer class is at the core of Report Server. Although it is documented as a Java interface, it is not intended to be replaced by a custom implementation built by developers. This section will talk about it as a Java class. The class provides methods to start and stop Report Server, to get information about deployed resources and to run and schedule reports. You can also save reports  as resources using these methods.
...
This class is used by client programs that run a report on a remote Report Server but then want the generated exported output such as PDF or Excel file to be available to open locally on the client computer. 
...
The Cluster class provides methods to enable the user to build their own algorithm for balancing server loads by assigning new reports to run onto the server you want.
...
This enables applications to configure the server similar to the Report Server Console. For programming examples use the actual JSP pages under public_html\admin. 
...
Report Server Monitor is a client application that can be downloaded for free from the Report downloads page. It enables you to monitor activity on a single server or on an entire group of servers in a cluster. The API can be used to start and stop servers as well as configure properties on them remotely. 
...
This is used by Report Server to implement Server Monitor. It enables your application to catch the same types of resource use as Server Monitor using JMX technology. 
